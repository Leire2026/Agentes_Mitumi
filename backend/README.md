...
- **📍 Puerto:** `5004` (Convivencia: Express `3000`, gateway `5003`).
...
### 🏥 Healthcheck
- `GET /health` → Sonda uniforme del sistema (`{estado, hora}`), la que usan el gateway y el smoke test.
- `GET /` → Verifica que el backend está "activo" y lista todas sus rutas disponibles.

> Si la base de datos no está disponible, el servicio responde **503** con
> `{"ok": false, "message": "Base de datos no disponible", ...}` — un error de
> conexión nunca se disfraza de "lista vacía".
