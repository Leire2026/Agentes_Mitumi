...
`kit_conexion_agentes` oficial del proyecto (mismo patrón que Lumen en producción) — nunca
escribe en ella. Los bloques `evento`, `cliente` y `ponentes` reutilizan los nombres de columna
reales de `base_datos/*.csv`; el bloque `nota_bene` (ver sección 3)
...
> 1. **Pedir al equipo de data la cadena de conexión `agente_readonly`** y ponerla en `.env` como
...
>    `python kit_conexion_agentes/test_conexion.py .env` (en el repositorio de desarrollo)
>    debe dar todo `PASS`.
...
```bash
cd agentes/Operis_autocompletado
python main.py --demo
```
...
> lee la BD real (Neon Postgres) en modo solo lectura, usando el `kit_conexion_agentes`
> del proyecto (`kit_conexion_agentes/`, en el repositorio de desarrollo) — el mismo patrón que usa
...
> Nota: esta tabla describe el contrato **interno** (`ejecutar_agente(payload)`).
> En la capa HTTP (`servidor.py`), `id_evento` es **opcional**: sin él, el agente
> procesa una extracción inicial sin histórico (ver más arriba).
...
Existe un kit de conexión oficial del proyecto, `kit_conexion_agentes`
(`kit_conexion_agentes/`, en el repositorio de desarrollo), ya usado en producción por Lumen
(`Lumen_buscador/integrations/db_backend.py`). `agente_operis` ahora lo usa
también:
...
- **Paso manual pendiente, no técnico:** hace falta pedir al equipo de data la cadena de conexión
  del rol `agente_readonly` (nunca la de `neondb_owner`) y ponerla en `.env` como
  `DATABASE_URL`. Verificación: `python kit_conexion_agentes/test_conexion.py .env`
  (en el repositorio de desarrollo) debe dar todo `PASS`.

**⚠️ Limitación real de la BD, no de este código:** según el esquema documentado
(`Lumen_buscador/data/rag/documentos/esquema_bd.md`), cada evento enlaza como
...
│                                    adaptado de kit_conexion_agentes (repositorio de desarrollo)
│
├── src/
...
DATABASE_URL=       # cadena del rol agente_readonly -- pídesela al equipo de data, NUNCA neondb_owner
```
...
- [ ] Pendiente, no técnico: pedir al equipo de data la cadena `agente_readonly` y ponerla en `.env` — sin eso, lo de arriba está construido pero sin probar contra la BD real (sí probado el camino "sin BD disponible", que degrada con elegancia).
